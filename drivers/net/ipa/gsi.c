...
static int gsi_channel_setup_one(struct gsi *gsi, u32 channel_id)
{
	...
	if (channel->toward_ipa)
		netif_napi_add_tx_weight(gsi->dummy_dev, &channel->napi,
					 gsi_channel_poll, NAPI_POLL_WEIGHT);
	else
		netif_napi_add_weight(gsi->dummy_dev, &channel->napi,
				      gsi_channel_poll, NAPI_POLL_WEIGHT);
...
err_irq_exit:
	gsi_irq_exit(gsi);
err_iounmap:
	/* Adjust register range pointer upward for newer IPA versions */
	iounmap(gsi->virt + (gsi->version < IPA_VERSION_4_5 ? 0 : GSI_EE_REG_ADJUST));
err_free_dummy:
